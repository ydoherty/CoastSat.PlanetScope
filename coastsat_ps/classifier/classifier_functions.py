...
from interactive import rescale_image_intensity


# coastsat modules
sys.path.insert(0, os.path.abspath(os.path.join(os.getcwd(), '..', '..')))
from shoreline_tools import (calculate_features, classify_image_NN, 
                             nd_index, get_ps_data)
...
def get_ps_metadata(filepath_images, epsg):
    ...
    for file in os.listdir(filepath_images):
        if file[-7:] == 'TOA.tif':
            metadata['PS']['filenames'] += [file]
    metadata['PS']['filenames'].sort()
    
    # Add other file data
    for file in metadata['PS']['filenames']:
        metadata['PS']['acc_georef'] += [5]
        metadata['PS']['epsg'] += [epsg]
        metadata['PS']['dates'] += [
            pytz.utc.localize(datetime(int(file[:4]), 
                                       int(file[5:7]), 
                                       int(file[8:10]), 
                                       int(file[11:13]), 
                                       int(file[14:16]), 
...
#%% Modified CoastSat Functions - Labelling - SDS Classify
...
def label_images(metadata,settings):
    ...
    Modified for PS data YD WRL 2020
    ...
    for satname in metadata.keys():
        ...
        for i in range(len(filenames)): 
            ...
            im_ms, cloud_mask = get_ps_data(fn)            
            ...
            # otherwise label this image
                ...
                for key in settings['labels'].keys():
                    ...
                    features[key] = calculate_features(im_ms, cloud_mask, im_bool)
...
def evaluate_classifier(classifier, classifier_save_name, metadata, settings):
    """
    Apply the image classifier to all the images and save the classified images.

    KV WRL 2019
    
    Modified for PS data YD 2020
    ...
    # create sub folder of classifier
    fp = os.path.join(fp, classifier_save_name)
    if not os.path.exists(fp):
        os.makedirs(fp)
    ...
    colours[1,:] = np.array([204/255,1,0,1])
    ...
    for satname in metadata.keys():
        ...
        # load classifiers and loop through images
        for i in range(len(filenames)):   
            # image filename
            fn = os.path.join(filepath, filenames[i])
            # read and preprocess image
            ...
            im_classif = classify_image_NN(im_ms, cloud_mask, classifier)
            ...
            im_sand = morphology.remove_small_objects(im_sand, min_size=min_beach_area_pixels, connectivity=2)
            im_water = morphology.remove_small_objects(im_water, min_size=min_beach_area_pixels, connectivity=2)
            ...
            # save figure
            fig.savefig(os.path.join(fp,settings['inputs']['sitename'] + '_' + filename[:19] +'.jpg'), dpi=150)
            # clear axes
            for cax in fig.axes:
               cax.clear()
